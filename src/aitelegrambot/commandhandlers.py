...
class NormalCommandHandlers(CommandHandlers):
    ...
    async def cringe_message(
            self,
            update: Update,
            context: ContextTypes.DEFAULT_TYPE,
    ):
        print("Cringe message")
        """
        Performs inference on the given update with both image and text query.
        ...
        """
        # Extract the query from the text message if available
        query: str = self.get_content(update.message.caption) if update.message.caption else None
        prev_msg_size: int = 0
        # Check if an image is attached
        image = update.message.photo[-1] if update.message.photo else None

        # Handling when both query and image are not provided
        if query is None and image is None:
            await update.message.reply_text(
                text="Please provide a text or attach an image to process.",
                parse_mode="Markdown"
            )
            return
        ...
        if image:
            ...
            # Here you would process the image and possibly use 'query' as additional context.
            # You need to replace the placeholder below with your actual image processing code.
            if query is None:
                query = generate_promt_cringe(image_path)
            else:
                query = generate_promt_cringe(image_path) + " also use this" + query
            # Handle text query
    
        if query:
            print("ask model")
            # Use your model to process text query
            stream_send = True
            if stream_send == False:
                query_response = self.ollama_state.client.chat(
                ...
                description += f"{query_response}"
 
                print("Send responce")
                # Final reply with the combined results of image and query processing
